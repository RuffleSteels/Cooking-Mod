...
public class KitchenBenchShaped extends CraftingRecipe {
    final int width;
    final int height;
    ...
    static String[] removePadding(String ... pattern) {
        ...
        for (int m = 0; m < pattern.length; ++m) {
            ...
            i = Math.min(i, KitchenBenchShaped.findFirstSymbol(string));
            int n = KitchenBenchShaped.findLastSymbol(string);
            j = Math.max(j, n);
            if (n < 0) {
                if (k == m) {
                    ++k;
    ...
    public static class Serializer
            implements RecipeSerializer<KitchenBenchShaped> {
        @Override
        public KitchenBenchShaped read(Identifier identifier, JsonObject jsonObject) {
            String string = JsonHelper.getString(jsonObject, "group", "");
            Map<String, Ingredient> map = KitchenBenchShaped.readSymbols(JsonHelper.getObject(jsonObject, "key"));
            String[] strings = KitchenBenchShaped.removePadding(KitchenBenchShaped.getPattern(JsonHelper.getArray(jsonObject, "pattern")));
            int i = strings[0].length();
            int j = strings.length;
            DefaultedList<Ingredient> defaultedList = KitchenBenchShaped.createPatternMatrix(strings, map, i, j);
            ItemStack itemStack = ShapedRecipe.outputFromJson(JsonHelper.getObject(jsonObject, "result"));
            return new KitchenBenchShaped(identifier, string, i, j, defaultedList, itemStack);
        }

        @Override
        public KitchenBenchShaped read(Identifier identifier, PacketByteBuf packetByteBuf) {
        ...
        @Override
        public void write(PacketByteBuf packetByteBuf, KitchenBenchShaped shapedRecipe) {
